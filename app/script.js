...
exportMdBtn.addEventListener("click", async () => {
  ...
  buildMd(null, 1);
  downloadFile(output, "branchpad.md", "text/markdown");
});

exportJsonBtn.addEventListener("click", async () => {
  const notes = await getAllNotes();
  downloadFile(JSON.stringify({ exportedAt: new Date(), notes }, null, 2), "branchpad.json", "application/json");
});
...
// ==========================
// Úvodní poznámka (jen při prvním spuštění)
// ==========================

async function createWelcomeNoteIfEmpty() {
  const notes = await getAllNotes();
...
• Novou poznámku vytvoříte tlačítkem „Nová poznámka“
• Podpoznámku vytvoříte pod vybranou poznámkou
• Poznámky se ukládají automaticky

Doporučení: pravidelně exportujte JSON jako zálohu.
